...
- **One home per fact, no redundant retelling**: `DECISIONS.md` owns the full reasoning/root-cause narrative for a decision; `TASKS.md` task notes stay terse — what happened, current status, a pointer to the `DECISIONS.md` entry by timestamp — rather than restating that narrative. The same principle applies across other docs (e.g. `README.md` should point to `CHEATSHEET.md` for command-level detail instead of duplicating it): a fact lives in exactly one file, everything else links to it instead of copying it.
...
- VS Code and repository editing may run on Windows. The deployment-only self-hosted runner, Docker, and k3d run on a disposable Hyper-V Ubuntu Server VM (`infra/hyperv-runner/`) — not WSL, which was decommissioned 2026-07-28 (see `DECISIONS.md`); disposable GitHub-hosted Ubuntu runners execute pull-request CI. Native Windows/OneDrive `npm ci` is not an authoritative clean-build environment (see `KNOWN_ISSUES.md`).
- Untrusted pull-request code must never run on the deployment-only self-hosted runner. Pull requests use GitHub-hosted runners; GHCR publishing is limited to successful `master` pushes; the later self-hosted-runner job consumes the verified registry artifact from a trusted ref.
