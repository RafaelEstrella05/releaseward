...
### [2026-07-29 19:51] Observability stack confirmed live on the Hyper-V k3d cluster

**Decision**: Close out the "Production observability" task's open follow-up from the 2026-07-28 09:45 entry. The user applied `k8s/observability/*.yaml` for real against the live `releaseward-dev` namespace on the Hyper-V deploy runner (`opsadmin` account, cluster-admin kubeconfig — not the restricted `releasewardrunner` deploy identity), using a throwaway `git clone --depth 1` of the public repo directly onto the runner rather than transferring individual files. All 7 resources (`configmap/grafana-dashboard-releaseward`, `deployment.apps/grafana`, `configmap/grafana-datasources`, `configmap/grafana-dashboard-providers`, `service/grafana`, `configmap/prometheus-config`, `deployment.apps/prometheus`, `service/prometheus`) created cleanly and both pods reached `1/1 Running`.

Verified through the firewall via SSH local port-forwarding (PuTTY tunnels, since `infra/hyperv-runner/bootstrap-post-install.sh` locks `ufw` to SSH-only — no port was opened to check this, the tunnel rode the existing SSH session) plus `kubectl port-forward` on the runner itself: Prometheus's `/targets` page shows `releaseward-demo` scraping `1/1 up` with ~5ms scrape duration; the Grafana dashboard (`releaseward demo service`, anonymous Viewer access, no login prompt) renders real data on 3 of its 4 panels — request rate, error rate (5xx), and p95 latency all show live traffic from the `/livez`/`/readyz` probes and `/metrics` scrapes. The 4th panel, classified security events by category, correctly shows "No data," since nothing has called `/classify` yet — not a defect, just an unexercised counter.

**Snag noted along the way**: `CHEATSHEET.md` — the repo's designated home for "poking around by hand" commands — was never updated with a Prometheus/Grafana section when the manifests were added on 2026-07-28, so there was no documented port-forward/tunnel procedure to follow. Worked around ad hoc this session; not yet fixed in the doc itself.
