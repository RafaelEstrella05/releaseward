...
Flow: GitHub-hosted Actions runs tests + security/dependency scans → builds and scans a Docker image → pull requests stop after verification, while a successful protected `master` push publishes the immutable commit-SHA image to GHCR → a deployment-only self-hosted runner pulls that verified artifact into local k3d → runs health checks (`readyz`/`livez`) → Claude reads trusted pipeline evidence and produces a human-readable release summary / anomaly flag → the release summary is posted as a PR/commit comment; structured logs/traces are captured throughout for real observability.
...
- **Runner trust boundary**: Public pull-request code runs only on disposable GitHub-hosted runners with no route to k3d. The WSL runner is reserved for trusted deployment from the protected default branch; it is not a general CI worker.
